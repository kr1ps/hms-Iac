...
resource "vsphere_virtual_machine" "vm" {
  ...
  provisioner "local-exec" {
   command = "tf_environment=${terraform.workspace} ansible-playbook -u '${var.ssh_user}' -i '${vsphere_virtual_machine.vm.default_ip_address},' ./ansible-hms/hms.yml --key-file '~/.ssh/id_ed25519'"
  }
}
